...
# -*- coding: utf-8 -*-
"""
ربات نرخ‌های طلا، ارز، نقره و کریپتو به تومان
- هر ۱۲ ساعت (ساعت ۰۹:۰۰ و ۲۱:۰۰ به وقت تهران) نرخ‌ها را می‌گیرد و در تلگرام می‌فرستد.
...
TOKEN = os.environ.get("TELEGRAM_TOKEN", "8857661304:AAFq5euAu_lpQHRwgW4eDiHC7hcQgdnhN9M")
...
def build_message():
    ...
    silver999 = tgju_today("silver_999")            # گرم نقره ۹۹۹ (ریال) ← سایت گرمی
    silver925 = tgju_today("silver_925")            # گرم نقره ۹۲۵ (ریال)
    ...
    lines = [
        ...
        "🔄 این پیام هر ۱۲ ساعت (ساعت ۹ صبح و ۹ شب) به‌روزرسانی می‌شود",
    ]
    return "\n".join(lines)
...
def next_run_time(now_tz):
    """بعدی ساعت ۰۹:۰۰ یا ۲۱:۰۰ به وقت تهران"""
    for hh in (9, 21):
        t = now_tz.replace(hour=hh, minute=0, second=0, microsecond=0)
        if t > now_tz:
            return t
    t = now_tz.replace(hour=9, minute=0, second=0, microsecond=0)
...
def main():
    ...
    if send_now or once:
        ...
        if once:
            return  # حالت یک‌باره (مثلاً برای GitHub Actions)

    while True:
        now = datetime.now(TEHRAN_TZ)
        nxt = next_run_time(now)
        wait = (nxt - now).total_seconds()
        print(f"[{now:%Y-%m-%d %H:%M:%S}] نوبت بعدی: {nxt:%Y-%m-%d %H:%M} "
              f"(انتظار {wait/3600:.1f} ساعت)", flush=True)
        time.sleep(wait)
        for attempt in range(3):
            try:
                ...
                print(f"خطا در ارسال (تلاش {attempt+1}): {e}", flush=True)
                time.sleep(30)
