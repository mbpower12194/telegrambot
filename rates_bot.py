...
def build_message(header: str = "💰📊 نرخ‌های لحظه‌ای بازار") -> str:
    ...
    hours_fa = "، ".join(fa_digits(f"{h:02d}:00") for h in SCHEDULE_HOURS)
    out += [
...
def handle_message(msg, subs: set) -> bool:
    ...
    try:
        if low == "/start":
            ...
            hrs = "، ".join(fa_digits(f"{h:02d}:00") for h in SCHEDULE_HOURS)
...
def align_to_hour(max_wait: int = 2100):
    ...
    target = min(
        (now.replace(hour=h, minute=0, second=0, microsecond=0) + d
         ...
         for d in (timedelta(0), timedelta(days=1), timedelta(days=-1))),
    ...
    if 0 < wait <= max_wait:
        ...
        log(f"⏱ کران با {int(-wait)} ثانیه تأخیر اجرا شد؛ فوری ارسال می‌شود.")
