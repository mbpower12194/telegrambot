...
def arrow(rate) -> str:
    ...
    if not pct:
        return ""
    sign = "🔺" if d == "high" else ("🔻" if d == "low" else "▪️")
...
def build_message(header: str = "💰📊 نرخ‌های لحظه‌ای بازار") -> str:
    ...
    out += ["📌 منبع: tgju.org  |  🔄 بروزرسانی خودکار هر ۶ ساعت"]
    return "\n".join(out)
...
HELP_TEXT = (
    ...
    "❓ /help → همین راهنما\n\n"
    "🔄 به‌صورت خودکار هم ساعت‌های ۳ بامداد، ۹ صبح، "
    "۳ بعدازظهر و ۹ شب نرخ‌ها ارسال می‌شود.\n"
...
def handle_message(msg, subs: set) -> bool:
    ...
    try:
        if low == "/start":
            ...
            reply = (body + "\n\n✅ از این پس هر ۶ ساعت هم برایتان ارسال می‌شود.\n"
